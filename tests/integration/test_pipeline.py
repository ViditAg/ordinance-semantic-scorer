...
Unlike unit tests that isolate one function at a time, these tests exercise the
**wiring** between ``app.utils.chunk_text`` and ``app.scorer.OrdinanceScorer`` using
the real ``app/criteria.json`` rubric. Heavy externals stay mocked:

* ``SentenceTransformer`` — replaced with deterministic Gaussian normalised vectors.
* ``pdfplumber`` — **not** mocked in the sample-PDF smoke section; those tests hit
  the real parser (skipped automatically if the PDF asset is absent).

**What "integration" means here**

We do **not** spin up Streamlit; instead we mimic the app's data flow:

``criteria.json → chunk_text → embed_texts → score``

and assert structural invariants (counts, key presence, numeric ranges) that would
be expensive to re-validate manually whenever the rubric JSON changes.
"""
...
# tests/integration → parents[2] is the repository root containing ``app/`` and ``sample/``.
...
class TestCriteriaJson:
    """Static structure checks for the bundled rubric file."""

    def test_file_exists(self):
        """Fail fast with a clear path if someone deletes criteria from the tree."""
        assert CRITERIA_PATH.exists(), f"criteria.json not found at {CRITERIA_PATH}"

    def test_file_is_valid_json(self):
        """Criteria ship as JSON (not YAML) so Streamlit's ``json.load`` stays aligned."""
        data = json.loads(CRITERIA_PATH.read_text(encoding="utf-8"))
        assert isinstance(data, dict)

    def test_top_level_key_is_criteria(self):
        """The UI loader expects a top-level ``criteria`` array — enforce that contract."""
        data = json.loads(CRITERIA_PATH.read_text(encoding="utf-8"))
    ...
    def test_expected_criterion_count(self):
        """
        Bump ``EXPECTED_CRITERIA_COUNT`` intentionally whenever the rubric grows/shrinks
        so reviewers see a conscious change in PR diffs.
        """
        data = json.loads(CRITERIA_PATH.read_text(encoding="utf-8"))
    ...
    def test_all_weights_are_positive(self):
        """Zero/negative weights would distort normalisation — disallow silently bad data."""
        data = json.loads(CRITERIA_PATH.read_text(encoding="utf-8"))
    ...
    def test_serial_numbers_are_unique(self):
        """Optional ``serial_number`` field must remain a bijection when present."""
        data = json.loads(CRITERIA_PATH.read_text(encoding="utf-8"))
...
class TestChunkingPipeline:
    """Lightweight checks that synthetic ordinance prose splits cleanly."""
    ...
    def test_chunking_produces_chunks(self):
        """Repeated boilerplate text should exceed one 400-char window."""
        chunks = chunk_text(self.SAMPLE_TEXT, chunk_size=400, overlap=50)
        assert len(chunks) > 1

    def test_chunk_count_consistent_across_calls(self):
        """Deterministic splitter → identical hyperparameters yield identical counts."""
    ...
    def test_all_chunks_non_empty(self):
        """No ``strip()``-only empty windows should leak into downstream embed calls."""
        chunks = chunk_text(self.SAMPLE_TEXT, chunk_size=400, overlap=50)
...
class TestFullPipeline:
    ...
    def _patch_model(self):
        """Swap the transformer for a tiny deterministic encoder (16-D vectors)."""
    ...
    @pytest.fixture()
    def criteria(self):
        """Fresh list object loaded from disk for each test function."""
        data = json.loads(CRITERIA_PATH.read_text(encoding="utf-8"))
    ...
    def pipeline_result(self, criteria):
        """
        Run the same sequence the Streamlit app uses (minus UI), with ``top_k=2``.

        Assertions consume this dict to ensure the full stack stays internally consistent.
        """
    ...
    def test_per_criterion_scores_in_range(self, pipeline_result):
        """No individual line item may report impossible scores."""
    ...
    def test_top_k_excerpts_count(self, pipeline_result):
        """Fixture used ``top_k=2`` — never return more excerpts than requested."""
        for r in pipeline_result["criteria_results"]:
            assert len(r["top_excerpts"]) <= 2

    def test_top_excerpts_are_strings(self, pipeline_result):
        """UI Markdown rendering expects plain strings, not bytes or dicts."""
        for r in pipeline_result["criteria_results"]:
            for excerpt in r["top_excerpts"]:
                assert isinstance(excerpt, str)

    def test_doc_embeddings_count_equals_chunk_count(self, criteria):
        """One embedding vector per chunk — misalignment would break similarity tables."""
        chunks = chunk_text(self.SAMPLE_TEXT, chunk_size=500, overlap=50)
        scorer = OrdinanceScorer(criteria=criteria, model_name="mock-model")
        doc_embeddings = scorer.embed_texts(chunks)
...
class TestRealSamplePdf:
    ...
    def extracted_text(self):
        """
        Expensive I/O (disk + pdf parsing) runs once per test class, not per method.

        Import happens inside the fixture to keep module import time low when the PDF
        is missing and the entire class is skipped.
        """
    ...
    def test_pdf_extraction_returns_non_empty_string(self, extracted_text):
        """Sanity check that pdfplumber can still read the committed sample asset."""
    ...
    def test_all_chunks_are_non_empty_strings(self, chunks):
        """Chunker should never emit whitespace-only segments for this PDF."""
